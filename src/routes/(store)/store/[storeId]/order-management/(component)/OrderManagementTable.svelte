...
<script>
	import OrderOperationModal from './OrderOperationModal.svelte';
	...
	let actionSelects = ['VIEW', 'EDIT'];

	export let tableTarget = 'table-data';

	export let store_id;

	let shownModalIndex = null; // no modal is shown initially

	function handleClick(index) {
		shownModalIndex = index; // show the modal with this index
	}
	...
	onMount(() => {
		let table = new DataTable(`#${tableTarget}`, {
			lengthChange: false,
			searching: true,
			ordering: true,
			info: true,
								...
								<Model modalTargetId={`modal-delete-${index}`} modalTitle={'ลบข้อมูล'}>
									<slot name="delete-action">
										...
										<div class="modal-footer">
											<form
												method="POST"
												action="?/deleteCategoryProduct"
												use:enhance={({ formData }) => {
													formData.append('product_category_id', record[1].split('-')[1]);
